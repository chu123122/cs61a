    ...
    def helper(tree):
        if tree.is_leaf():
            ...
            for t in tree.branches:
                list+=helper(t)
            return [t.label for t in tree.branches]+list
    if not t.is_leaf():
        for number in helper(t):
            t.label*=number
        for b in t.branches:
            cumulative_mul(b)
    #   for b in t.branches:
...
def convert_link(link):
    ...
    if link is not Link.empty:
        if isinstance(link.first,Link):
            if link.first is not Link.empty:
                list+=[link.first.first]+convert_link(link.first.rest)+convert_link(link.rest)
        else:
            list+=[link.first]+convert_link(link.rest)
    return list
