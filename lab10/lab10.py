...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    result,div=args.first,args.rest
    while div is not nil:
        result//=div.first
        div=div.rest
    return result
...
def eval_and(expressions):
    ...
    if expressions is nil:
        return True
    check,rest=expressions.first,expressions.rest
    while rest is not nil:
        if check is scheme_f:
            return scheme_f
        else:
            rest=eval_and(rest.map(calc_eval))
            return rest
    return check
...
def eval_define(expressions):
    ...
    bindings[name]=expressions.rest.first
    return name
