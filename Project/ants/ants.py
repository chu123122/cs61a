...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            original=place.ant.can_contain(self)
            current=self.can_contain(place.ant)
            if current==original:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if 0>=self.lower_bound and 0<=self.upper_bound:
            near=random_bee(self.place.bees)
        else:
            near=None
        def helper(place,bound):
            next_place=place.entrance
            if not next_place.is_hive:
                nea=random_bee(next_place.bees)
                in_bound= (bound >=self.lower_bound and bound<=self.upper_bound)
                if nea is None or not in_bound:
                    return helper(next_place,bound+1)
                else:
                    return nea
            else:
                return None
        return near if near is not None else helper(self.place,1)
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        ...
        while current_place is not None:
            ...
            if current_ant is not None:
                current_ant.double()
                if current_ant.is_container and current_ant.ant_contained is not None:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem EC 4
        current_place,dic,dis=self.place.entrance,{},1
        if self.place.ant.is_container:
            dic[self.place.ant]=0
        for bee in self.place.bees:
            if bee is not None:
                dic[bee]=0
                
        while current_place is not None:
            ant,bees=current_place.ant,current_place.bees
            for bee in bees:
                if bee is not None:
                    dic[bee]=dis
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            print('DEBUG','攻击中',insect,distance)
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem EC 3
        current_ant=self.place.ant
        if current_ant is None or current_ant.blocks_path is False:
            return False
        else:
            return True
        # END Problem EC 3
