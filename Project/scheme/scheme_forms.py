...
def do_quote_form(expressions, env):
    ...
    if expressions.rest is nil:
        ...
        return expressions.map(lambda expr:scheme_eval(expr,env))
    # END PROBLEM 5
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if result is None and clause.rest is not nil:
                return None
...
def make_let_frame(bindings, env):
    ...
    bindings_pair=bindings
    while bindings_pair is not nil:
        pair=bindings_pair.first    
        validate_form(pair,2,2)
        names,vals=Pair(pair.first,names),Pair(scheme_eval(pair.rest.first,env),vals)
        bindings_pair=bindings_pair.rest
