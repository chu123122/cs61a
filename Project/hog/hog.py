...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    ones,tens=player_score%10,opponent_score//10
    if tens>=10:
        tens%=10
    sum=3*abs(ones-tens)
    if sum<2:
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    mount=num_factors(score)
    if mount>=3 and mount<=4:
        number=score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    score0,score1=0,0
    const=strategy(score0,score1)
    while score0!=goal:
        while score1!=goal:
            if strategy(score0,score1)!=const:
                return False
            score1+=1
        score0+=1
        score1=0
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    overage=make_averaged(roll_dice,samples_count)
    mount,count=1,0
    target=0
    while not mount>10:
        current=overage(mount,dice)
        if count<current:
            count=current
            target=mount
        mount+=1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    sus_update_points=sus_update(0,score,opponent_score)-score

    if sus_update_points>=threshold:
        return 0
    return num_rolls
    # END PROBLEM 11
