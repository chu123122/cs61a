...
class BankAccount:
    ...
    def deposit(self, amount):
        ...
        """
        self.transactions+=[Transaction(self.count,self.balance,self.balance+amount)]
        self.count+=1
        self.balance = self.balance + amount
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        """
        if amount > self.balance:
            self.transactions+=[Transaction(self.count,self.balance,self.balance)]
            self.count+=1
            return 'Insufficient funds'
        self.transactions+=[Transaction(self.count,self.balance,self.balance-amount)]
        self.count+=1
...
def make_change(amount, coins):
    ...
    if amount==smallest:
        ...
        next=make_change(amount-smallest,rest)
        if next is None:
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if self.coins.get(coin) is None:
            ...
            self.coins[coin]+=1
        coins_list,detele_list=make_change(coin,self.coins),[]
        for i in coins_list:
            self.coins[i]-=1
            if self.coins[i]==0:
                detele_list+=[i]
        for de in detele_list:
            self.coins.pop(de)
        return coins_list
