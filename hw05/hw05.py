...
class AggressivePlayer(Player):
    ...
    """
    def __init__(self, name, random_func):
        super().__init__(name, random_func)
...
def level_mutation_link(t, funcs):
	...
	if len(t.branches)==0 and remaining is not Link.empty:
...
def store_digits(n):
    ...
    def helper(link,n):
        rest=n//10
        if rest>1:
            link.rest=Link(rest%10)
            helper(link.rest,rest)
    def change_link(li,rest):
        if li.rest is Link.empty:
            return Link(li.first,rest)
        else:
            rest=Link(li.first,rest)
            return change_link(li.rest,rest)
        
    helper(lin,n)
    return change_link(lin,Link.empty)
